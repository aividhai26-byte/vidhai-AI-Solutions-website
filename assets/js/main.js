...
(function ($) {
	...
	if ($("#contact-form").length > 0) {
		$("#contact-form").validate({
			...
			submitHandler: function (form) {
				handleEmailJSSubmit(form, null, "service_4wlmlx9", "template_9ouv6eh", function() {
					if ($("#message_sent").length > 0) {
						$("#message_sent").modal("show");
					} else {
						alert("Message Sent! Thank you for contacting Vidhai AI Solutions.");
					}
				}, function(err) {
					if ($("#message_fail").length > 0) {
						$("#message_fail").modal("show");
					} else {
						alert("Oops! Something went wrong while sending. Please check your network and try again.");
					}
	...
	/*---------------------------------------------------------
	 Support Chatbot Dynamic Injection & Logic (Active Chat Mode)
	---------------------------------------------------------*/
	$(document).ready(function() {
		if ($("#vidhai-chatbot-bubble").length === 0) {
			const chatbotHTML = `
				<div id="vidhai-chatbot-bubble" class="chatbot-bubble">
					<i class="fa-solid fa-comments"></i>
				</div>
				<div id="vidhai-chatbot-window" class="chatbot-window">
					<div class="chatbot-header">
						<div class="chatbot-header-info">
							<div class="chatbot-avatar">V</div>
							<div>
								<div class="chatbot-name">Vidhai AI Support</div>
								<div class="chatbot-status">Online • Agent Bot</div>
							</div>
						</div>
					...
					<div class="chatbot-messages" id="chatbot-messages-container">
						<div class="chat-message bot-message">
							Hello! 👋 Welcome to Vidhai AI Solutions. I am here to help. You can type any question below (like asking about our "location", "services", "custom software/ERP", "internships"), or choose a quick option chip:
						</div>
					</div>
					<div class="chatbot-options" id="chatbot-options-container">
					</div>
					<form id="chatbot-input-form" class="chatbot-input-area">
						<input type="text" id="chatbot-user-input" class="chatbot-input" placeholder="Type your question here..." autocomplete="off" required>
			...
			const chatbotBubble = $("#vidhai-chatbot-bubble");
			const chatbotWindow = $("#vidhai-chatbot-window");
			const chatbotCloseBtn = $("#chatbot-close-btn");
			const messagesContainer = $("#chatbot-messages-container");
			const optionsContainer = $("#chatbot-options-container");
			const inputForm = $("#chatbot-input-form");
			...
			const chatbotData = {
				menu: [
					...
					{ text: "📍 Location", next: "location" },
					{ text: "⚙️ Custom CRM / ERP", next: "custom_software" },
					{ text: "🎓 Internships / Training", next: "internships" },
					{ text: "📞 Contact Info", next: "contact" }
				],
				services: {
					reply: "We specialize in Enterprise AI & digital transformation:\n• Generative AI & custom LLMs\n• Web Dev & Full Stack (MERN)\n• Custom software, CRMs & School ERPs\n• AI Chatbots & NLP models\n• Machine Learning, OCR & vision systems\n• Digital Marketing & SEO optimization",
					options: [{ text: "Back to Main Menu", next: "main_menu" }]
				},
				location: {
					reply: "Our office is located at:\n📍 7, Sidco Road, Vijayapuram Post, Tiruppur, Tamil Nadu, India - 641606.\nOpen Mon-Sat, 9am - 7pm.",
					options: [{ text: "Back to Main Menu", next: "main_menu" }]
				},
				custom_software: {
					reply: "We build bespoke software systems (like Construction CRMs & School ERPs) tailored to your exact workflows. Custom builds typically take 4-8 weeks. We offer dedicated system testing and support SLA agreements.",
					options: [
						{ text: "Request Consultation", next: "request_consultation" },
						{ text: "Back to Main Menu", next: "main_menu" }
					]
				},
				internships: {
					reply: "Yes! We run hands-on, project-based engineering internships in MERN stack, Data Science, and Applied AI. We also hold custom LinkedIn profile optimization, resume writing, and Power BI bootcamps at academic colleges.",
					options: [{ text: "Back to Main Menu", next: "main_menu" }]
				},
				contact: {
					reply: "You can reach us directly:\n📞 Call: +91 9345504199\n✉️ Email: aividhai26@gmail.com\nOr fill out the form on our Contact page!",
					options: [{ text: "Back to Main Menu", next: "main_menu" }]
				},
				request_consultation: {
					reply: "Great! Click the button below to go to our Contact page, or call us at +91 9345504199.",
					options: [
						{ text: "Go to Contact Page", action: "link_contact" },
						{ text: "Back to Main Menu", next: "main_menu" }
					]
				}
			};
			...
			function showMainMenu() {
				...
				chatbotData.menu.forEach(item => {
					optionsContainer.append(`<button type="button" class="chatbot-option-chip" data-next="${item.next}">${item.text}</button>`);
				});
			}
			...
			function appendMessage(text, isBot = true) {
				...
				messagesContainer.append(`<div class="chat-message ${msgClass}">${formattedText}</div>`);
				messagesContainer.scrollTop(messagesContainer[0].scrollHeight);
			}

			// Show Main Menu options on start
			showMainMenu();

			// Toggle open/close
			chatbotBubble.on("click", function() {
			...
			// Handle Option Click
			optionsContainer.on("click", ".chatbot-option-chip", function() {
				const nextKey = $(this).data("next");
				...
				setTimeout(function() {
					if (nextKey === "main_menu") {
						appendMessage("What else can I help you with?");
						showMainMenu();
					} else {
						const data = chatbotData[nextKey];
						if (data) {
							appendMessage(data.reply);
							// Show reply options
							data.options.forEach(item => {
								if (item.action) {
									optionsContainer.append(`<button type="button" class="chatbot-option-chip" data-action="${item.action}">${item.text}</button>`);
								} else {
									optionsContainer.append(`<button type="button" class="chatbot-option-chip" data-next="${item.next}">${item.text}</button>`);
								}
							});
			...
			// Handle Chat Submission (Texting Mode)
			inputForm.on("submit", function(e) {
				e.preventDefault();
				const text = userInput.val().trim();
				if (!text) return;

				// 1. Append user's text message
				appendMessage(text, false);
				userInput.val("");

				// 2. Process query in lower case keywords
				const query = text.toLowerCase();
				
				setTimeout(function() {
					// Keyword match routing
					if (query.includes("service") || query.includes("offer") || query.includes("what you do") || query.includes("expert") || query.includes("work")) {
						appendMessage(chatbotData.services.reply);
						showMainMenu();
					} else if (query.includes("location") || query.includes("address") || query.includes("where") || query.includes("office") || query.includes("place")) {
						appendMessage(chatbotData.location.reply);
						showMainMenu();
					} else if (query.includes("erp") || query.includes("crm") || query.includes("custom") || query.includes("software") || query.includes("billing") || query.includes("school")) {
						appendMessage(chatbotData.custom_software.reply);
						showMainMenu();
					} else if (query.includes("intern") || query.includes("training") || query.includes("course") || query.includes("workshop") || query.includes("college") || query.includes("mou")) {
						appendMessage(chatbotData.internships.reply);
						showMainMenu();
					} else if (query.includes("contact") || query.includes("phone") || query.includes("email") || query.includes("call") || query.includes("number")) {
						appendMessage(chatbotData.contact.reply);
						showMainMenu();
					} else if (query.includes("hi") || query.includes("hello") || query.includes("hey") || query.includes("greetings") || query.includes("sup")) {
						appendMessage("Hello! 👋 How can I help you today? Feel free to ask about our custom ERP systems, internships, location, or services!");
						showMainMenu();
					} else {
						appendMessage("I am not sure I understand that. 😅 Try asking about our 'services', 'location', 'ERP systems', 'internships', or 'contact'. Or select one of the quick options below:");
						showMainMenu();
					}
				}, 500);
			});
		...
		// ==========================================================================
		// VIDHAI AI SOLUTIONS - EMAILJS INTEGRATION FOR INQUIRY SUBMISSIONS
		// ==========================================================================
		// Initialize EmailJS with User Public Key if SDK is loaded
		if (typeof emailjs !== "undefined") {
			emailjs.init("xJ5Iq5S4BkTpuRI9T");
		}

		// Unified submit handler with loading states and dynamic timestamps (exposed globally for validator compatibility)
		window.handleEmailJSSubmit = function(formElement, event, serviceId, templateId, successCallback, errorCallback) {
			if (event && event.preventDefault) {
				event.preventDefault();
			}
			if (typeof emailjs === "undefined") {
				alert("EmailJS SDK failed to load. Please reload the page.");
				return;
			}
			...
			if (submitBtn.find(".btn-text span").length > 0) {
				submitBtn.find(".btn-text span").text("Sending...");
			} else {
				submitBtn.text("Sending...");
			}

			// Inject current time in Kolkata time zone
			const localTime = new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
			form.find("input[name='time']").val(localTime);
		...
		// 1. Homepage "Tell Us Your Problem" Form (index.html)
		$("#homepage-problem-form").on("submit", function(e) {
			handleEmailJSSubmit(this, e, "service_4wlmlx9", "template_9ouv6eh", function() {
				if ($("#message_sent").length > 0) {
					$("#message_sent").modal("show");
				} else {
					alert("Thank you! Your custom request has been sent successfully. We will respond within 2 hours.");
				}
			}, function(err) {
				if ($("#message_fail").length > 0) {
					$("#message_fail").modal("show");
		...
		// 2. Service Page Contact Form (service.html)
		$("#svc-contact-form").on("submit", function(e) {
			handleEmailJSSubmit(this, e, "service_4wlmlx9", "template_9ouv6eh", function() {
				if ($("#message_sent").length > 0) {
					$("#message_sent").modal("show");
				} else {
